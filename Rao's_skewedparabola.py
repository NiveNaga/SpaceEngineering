...
def bell_nozzle(k, aratio, Rt, l_percent):
    ...
    ye = [];
    for i in angle_list:
        xe.append(0.5 * Rt * math.cos(i))
        ye.append(0.5 * Rt * math.sin(i) + 1.5 * Rt)
...
def plot_nozzle(ax, title, Rt, angles, contour):
    ...
    text = ' 0.5 * Rt = ' + str(round(0.5 * Rt, 1))
    ...
    # bell section
    ax.plot(xbell, ybell, linewidth=1.5, color='b')
    ax.plot(xbell, nybell, linewidth=1.5, color='b')
